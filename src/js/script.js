...
next.addEventListener('click', () => {
	...
	if (offset == deleteNotDidits(widthBlockSlider) * (slides.length - 1)) {
		offset = 0;
	} else {
		offset += deleteNotDidits(widthBlockSlider);
	};
...
prev.addEventListener('click', () => {
	// Проверяем что это последний слайд
	if (offset == 0) {
		offset = deleteNotDidits(widthBlockSlider) * (slides.length - 1);
	} else {
		offset -= deleteNotDidits(widthBlockSlider);
	};
...
dots.forEach(dot => {
	dot.addEventListener('click', (event) => {
		...
		slideIndex = slideTo;
		offset = deleteNotDidits(widthBlockSlider) * (slideTo - 1);
